...
export default function DesignedForClarity() {
  ...
  return (
    <section ref={sectionRef} className="bg-[#12141D] py-16 sm:py-24 px-4 sm:px-6 overflow-hidden">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">

        {/* Left — Phone */}
        <m.div ref={phoneRef} style={{ y: phoneY, rotateZ: phoneRotate }} className="flex justify-center opacity-0">
          <ClarityPhoneMockup />
        </m.div>

        {/* Right — Text */}
        <div className="flex flex-col gap-6">
          <m.p initial={{ opacity: 0, y: 10 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-[11px] font-bold text-[#1E8B4C] tracking-widest uppercase">
            Designed for Educational Institutions
          </m.p>
          <m.h2 initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.1 }} className="text-[2rem] sm:text-[2.6rem] font-bold text-white leading-tight tracking-tight">
            Built Around Institutional Communication
          </m.h2>
          <m.p initial={{ opacity: 0, y: 16 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }} className="text-[14px] text-white/55 leading-relaxed">
          ...
          <div className="flex flex-col gap-3 mt-2">
            {features.map((f, i) => (
              <m.div key={`clarity-${f}`} initial={{ opacity: 0, x: -16 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.3 + i * 0.1 }}
                ...
                <CheckCircle size={14} className="text-[#1E8B4C] flex-shrink-0 mt-0.5" />
...
function ClarityPhoneMockup() {
  ...
  return (
    <div className="relative w-[200px] sm:w-[240px]">
      <div className="absolute inset-0 bg-[#1E8B4C]/8 rounded-[36px] blur-3xl scale-110" />
      <div className="relative bg-[#1A1D28] border border-white/10 rounded-[28px] overflow-hidden shadow-2xl" style={{ aspectRatio: "9/19" }}>
        <div className="absolute top-3 left-1/2 -translate-x-1/2 w-14 h-4 bg-[#12141D] rounded-full z-10" />
        <div className="absolute inset-0 p-3 pt-10 flex flex-col gap-2">
          {/* Header */}
          <div className="flex items-center justify-between mb-1">
            <div className="size-20 bg-white/20 rounded-full" />
            <div className="size-5 rounded-full bg-[#1E8B4C]/30" />
          </div>
          {/* Notice card */}
          <div className="bg-[#1E8B4C]/10 border border-[#1E8B4C]/20 rounded-xl p-2.5">
            <div className="flex items-center gap-1.5 mb-1.5">
              <div className="size-1.5 rounded-full bg-[#1E8B4C]" />
              <div className="h-1.5 w-12 bg-[#1E8B4C]/60 rounded-full" />
            </div>
          ...
          {[...Array(4)].map((_, i) => (
            <div key={`item-${i}`} className="flex items-center gap-2 p-2 rounded-lg bg-white/4">
              ...
              <div className="size-4 rounded-full bg-[#1E8B4C]/30 flex-shrink-0" />
            </div>
          ))}
          {/* Bottom nav */}
          <div className="mt-auto flex justify-around pt-1 border-t border-white/5">
            {[...Array(4)].map((_, i) => (
              <div key={`item-${i}`} className="flex flex-col items-center gap-1">
                <div className={`size-4 rounded-md ${i === 0 ? "bg-[#1E8B4C]/40" : "bg-white/10"}`} />
